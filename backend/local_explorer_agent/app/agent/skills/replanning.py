...
class ReplanningSkill:
    ...
    def run(self, *, plan: PlanOutput, event: PlanEvent) -> PlanOutput:
        ...
        elif event.event_type == EventType.POI_CLOSED:
            changed = self._replace_affected_poi(updated, event)
            updated.replan_reason = (
                "地点临时关闭，已优先替换为该阶段备选 POI。"
                if changed
                else "地点关闭事件已记录，但没有找到可替换备选点。"
            )
        ...
        elif event.event_type == EventType.USER_PREFERENCE_CHANGE:
            updated.state = PlanState.FEEDBACK
            updated.replan_reason = "用户偏好发生变化，已进入反馈阶段等待下一轮局部调整。"
