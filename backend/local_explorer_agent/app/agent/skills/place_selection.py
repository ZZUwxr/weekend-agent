...
_INTENT_CATEGORY_MAP: dict[str, list[str]] = {
    ...
    "黑盒剧场": ["小剧场"],
    "亲子空间": ["亲子空间"],
    "情侣": ["展览", "Citywalk", "买手店", "手作体验", "咖啡", "甜品", "茶馆"],
    "约会": ["展览", "Citywalk", "买手店", "手作体验", "咖啡", "甜品", "茶馆"],
    "二人世界": ["展览", "Citywalk", "咖啡", "甜品", "茶馆"],
    "浪漫": ["公园", "甜品", "咖啡", "茶馆"],
    "展览": ["展览"],
    "看展": ["展览"],
    "逛展": ["展览"],
...
_NON_FAMILY_EXCLUDED_CATEGORIES = {"亲子空间", "游乐园"}
...
class PlaceSelectionSkill:
        ...
        *,
        ...
        for stage in candidate.stages:
            ...
            categories = _apply_stage_category_policy(
                ...
                allow_family_categories=_allows_family_categories(group_context),
            )
            ...
            if not pois:
                fallback_categories = _fallback_categories_for_stage(
                    stage_type,
                    allow_family_categories=_allows_family_categories(group_context),
                )
    ...
    def _poi_rank(
        ...
        *,
        ...
        closed_rank = int(start_time is not None and not _is_open_at(poi, start_time))
        memory_rank = -_memory_poi_score(poi, user_memory)
        return (
            category_match,
            closed_rank,
            memory_rank,
...
def _apply_stage_category_policy(
    *,
    ...
    allow_family_categories: bool = True,
) -> list[str]:
    base = _dedupe(categories or [])
    if not allow_family_categories and not priority_categories:
        base = [category for category in base if category not in _NON_FAMILY_EXCLUDED_CATEGORIES]
    if stage_type == "dine":
    ...
    if filtered:
        return filtered
    return _fallback_categories_for_stage(
        stage_type,
        allow_family_categories=allow_family_categories,
    )
...
def _fallback_categories_for_stage(
    stage_type: str,
    *,
    allow_family_categories: bool = True,
) -> list[str]:
    categories = list(_STAGE_FALLBACK_CATEGORIES.get(stage_type, []))
    if allow_family_categories:
        return categories
    return [category for category in categories if category not in _NON_FAMILY_EXCLUDED_CATEGORIES]


def _allows_family_categories(group_context: GroupContext) -> bool:
    query = group_context.input_query or ""
    if str(group_context.group_type) == "family":
        return True
    return any(token in query for token in ("孩子", "小孩", "娃", "宝宝", "女儿", "儿子", "亲子"))
...
def _is_open_at(poi: POI, start_time: datetime) -> bool:
    if not poi.open_hours:
        return True
    hours = poi.open_hours.strip().lower()
    if "全天" in hours or "24小时" in hours or "00:00-24:00" in hours:
        return True
    if "周一闭馆" in hours and start_time.weekday() == 0:
        return False
    if "休息" in hours or "closed" in hours:
        return False

    ranges = re.findall(
        r"(\d{1,2}):(\d{2})\s*[-–]\s*(?:次日)?(\d{1,2}):(\d{2})",
        hours,
    )
    if not ranges:
        return True
...
def _memory_poi_score(poi: POI, memory: UserMemoryContext | None) -> float:
    if memory is None:
        return 0.0
    score = 0.0
    if poi.id in memory.liked_poi_ids:
        score += 0.8
    if poi.id in memory.disliked_poi_ids:
        score -= 1.0
    score += (memory.category_weights.get(poi.category, 1.0) - 1.0) * 2
    ...
    for term in terms:
        score += (memory.tag_weights.get(term, 1.0) - 1.0) * 0.6
    if poi.category in memory.likes:
        score += 0.2
    if poi.category in memory.dislikes:
        score -= 0.3
