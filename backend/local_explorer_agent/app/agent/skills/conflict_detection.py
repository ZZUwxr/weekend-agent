...
class ConflictDetectionSkill:
    ...
    def _run_rule_based(self, group_context: GroupContext) -> list[Conflict]:
        ...
        if group_context.group_type == GroupType.COUPLE:
            return self._detect_couple_conflicts(group_context)
        if group_context.group_type == GroupType.SOLO:
            return self._detect_solo_conflicts(group_context)
    ...
    def _detect_couple_conflicts(self, group_context: GroupContext) -> list[Conflict]:
        role_ids = {role.role_id for role in group_context.roles}
        user_id = self._pick(role_ids, "adult_user")
        partner_id = self._pick(role_ids, "spouse_partner")
        conflicts = [
            Conflict(
                conflict_id="date_atmosphere_vs_execution",
                conflict_type=ConflictType.PACE_CONFLICT,
                involved_roles=[user_id, partner_id],
                description="情侣约会需要氛围和自然聊天，但排队、绕路或过度打卡会削弱体验。",
                severity=3,
                affected_decisions=[
                    DecisionType.ACTIVITY,
                    DecisionType.DINING,
                ...
                evidence=["输入包含情侣或约会意图", "约会场景对氛围和节奏敏感"],
                resolution_hint="用一个轻氛围开场、一个可聊天餐饮和一个低强度收尾，保持约会语境一致。",
            )
        ]
        spouse = next((role for role in group_context.roles if role.role_type == RoleType.SPOUSE), None)
        if spouse and any("低" in item or "轻食" in item for item in spouse.hard_constraints):
            conflicts.append(
                Conflict(
                    conflict_id="date_diet_balance",
                    conflict_type=ConflictType.DIET_CONFLICT,
                    involved_roles=[partner_id, user_id],
                    description="伴侣有低负担饮食需求，但约会也需要保留正常用餐氛围。",
                    severity=3,
                    affected_decisions=[DecisionType.DINING],
                    evidence=["输入提到减脂、低卡或清淡", "约会餐饮不能只像健康任务"],
                    resolution_hint="选择有氛围的轻食或多选项餐厅，把低卡作为自然选择。",
                )
            )
        return conflicts

    def _detect_solo_conflicts(self, group_context: GroupContext) -> list[Conflict]:
        role_ids = {role.role_id for role in group_context.roles}
        user_id = self._pick(role_ids, "adult_user")
        return [
            Conflict(
                conflict_id="solo_novelty_vs_rest",
                conflict_type=ConflictType.PACE_CONFLICT,
                involved_roles=[user_id],
                description="单人出行既需要一点新鲜感，也需要节奏自由和可随时调整。",
                severity=2,
                affected_decisions=[DecisionType.ACTIVITY, DecisionType.TIMELINE],
                evidence=["输入表达单人/自己出门", "单人路线不应被家庭或朋友模板绑住"],
                resolution_hint="安排一个轻探索点和一个低强度休息点，保持单人节奏自由。",
            )
        ]
    ...
    def _pick(self, role_ids: set[str], preferred: str) -> str:
        if preferred in role_ids or not role_ids:
            return preferred
