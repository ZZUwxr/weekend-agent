...
def test_negotiation_skips_when_no_conflicts() -> None:
    context = UserUnderstandingSkill().run(
        user_query="今晚只想去看个展，别安排别的",
        user_id="u003",
        city="深圳",
        start_time=datetime.fromisoformat("2026-05-11T19:00:00"),
        duration_minutes=180,
    )

    strategies = NegotiationSkill().run(group_context=context, conflicts=[])
